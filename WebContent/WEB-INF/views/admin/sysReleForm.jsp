...
<head>
	...
	function doSetDeptDic(id){
        ...
        Pku.getDom(id).src = "${ctx }/admin/sysReleDic?deptId=${_pku_user.deptId}&deptLevel=" + deptLevel + "&releType=1";
        Pku.getDom(id).focus();
	}
...
<body>
	<form id="addDeptFrm" url="${ctx}/admin/sysReleSave" method="post" onExtSuccess="doResult()" onExtFailure="doResult()">
		<input type="hidden" name="oldReleId" value="${sysDept.deptId}" />
		<div class="dept-edit">
			<div>
                <label><span>*</span>单位级别</label>
                <span id="radios" >
					<pku:dic dicName="DIC_RELE_LEVEL" var="releLevelItems">
					<c:forEach items="${releLevelItems}" var="item">
			...
			<div class="psr">
                ...
                <div class="psa" style="top:4px;left:95px;">
	                ...
	                <input type="text" class="in-text" id="parentDeptId" _name="parentDeptId" kind="dic" src="${ctx }/admin/sysReleDic?deptId=${_pku_user.deptId}&deptLevel=${sysDept.deptLevel}&releType=1" value="${sysDept.parentDeptId}" must="true" 
