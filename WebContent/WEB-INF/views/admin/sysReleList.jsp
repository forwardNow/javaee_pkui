...
<head>
	...
	var UUR = {
		// 新增
		'sysDeptAdd' : '${ctx}/admin/sysReleForm?' + Math.random(),
		// 修改
		'sysDeptUpdate' : '${ctx}/admin/sysReleForm?' + Math.random(),
		// 删除
		'sysDeptDelete' : '${ctx}/admin/sysReleDelete?' + Math.random(),
		//高级查询
		'sysDeptQuery'  : '${ctx}/admin/sysReleAdvQuery?' + Math.random(),
        //设置主管单位
        'setMainDept'  : '${ctx}/admin/sysDeptSetMainForm?' + Math.random(),
        //生成字典文件
        'sysDeptCreateDic'  : '${ctx}/admin/sysReleCreateDic?' + Math.random() 		
	};
	...
	function doSysDeptDelete(btn){
		var sel = Pku.getExt("sysDeptGrid").getSelectionModel().getSelections();
		if(sel.length > 0){
			Dialog.confirm("对应单位的数据归属也将被删掉，确认删除选中记录吗？", function(){
	            var urlParams = "";
	...
	function doSysDeptDic(){
        Pku.Common.ajax(UUR.sysDeptCreateDic,{dicName: 'DIC_RELE'},function(bln,response,o){
            var objRet = toJson(response.responseText);
            if(objRet.success){
		...
		<div xtype="colmodel">
			<div type="checkbox"></div>
		 	...
		 	<div header="单位名称" width="180" sortable="true" dataIndex="deptName"></div>
		  	...
		  	<div header="上级单位名称" width="180" sortable="true" dataIndex="parentDeptId" kind="dic" src="DIC_RELE"></div>
